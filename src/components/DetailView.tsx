...
import { X, BookOpen, Trash2, Edit3, Check, Minus, Plus } from 'lucide-react';
...
export default function DetailView({ book: initialBook, onClose }: Props) {
  ...
  // Two-phase mount animation: first render (visible=false), then animate in
  useEffect(() => {
  ...
  // Build gradient backgrounds for light and dark mode
  const heroGradient = darkMode
    ? `linear-gradient(180deg, 
        rgba(${dmr}, ${dmg}, ${dmb}, 0.7) 0%, 
        rgba(${dr}, ${dg}, ${db}, 0.4) 40%, 
        rgba(${mr}, ${mg}, ${mb}, 0.15) 75%, 
        var(--color-bg) 100%)`
    : `linear-gradient(180deg, 
        rgba(${vr}, ${vg}, ${vb}, 0.35) 0%, 
        rgba(${dr}, ${dg}, ${db}, 0.2) 40%, 
        rgba(${lvr}, ${lvg}, ${lvb}, 0.1) 75%, 
        var(--color-bg) 100%)`;

  const panelBg = darkMode
    ? `linear-gradient(180deg, 
        rgba(${dmr}, ${dmg}, ${dmb}, 0.12) 0%, 
        var(--color-bg) 100%)`
    : `linear-gradient(180deg, 
        rgba(${mr}, ${mg}, ${mb}, 0.08) 0%, 
        var(--color-bg) 100%)`;

  // Subtle glow behind the cover image
  ...
  // Accent color for UI elements (clamped for readability)
  ...
  // Soft tinted surface for cards within the detail view
  const tintedSurface = darkMode
    ? `rgba(${dmr}, ${dmg}, ${dmb}, 0.18)`
    : `rgba(${dr}, ${dg}, ${db}, 0.07)`;

  const tintedSurfaceStrong = darkMode
    ? `rgba(${dmr}, ${dmg}, ${dmb}, 0.28)`
    : `rgba(${dr}, ${dg}, ${db}, 0.12)`;
    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={handleClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {/* Hero Cover Area - Dynamic gradient from cover colors */}
          <div
          ...
          {/* Content area with subtle tinted background bleed */}
          <div
          ...
          >
            ...
            {/* Meta pills - tinted with cover color */}
            ...
            {book.status === 'currently-reading' && book.totalPages > 0 && (
              <div className="rounded-[20px] p-5" style={{ backgroundColor: tintedSurface }}>
                ...
                {/* Progress bar - tinted with cover accent */}
